...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, const Predicate& predicate) const {
        ...
        constexpr double x = (1e-6);

        sort(matched_documents.begin(), matched_documents.end(),
            [](const Document& lhs, const Document& rhs) {
                if (abs(lhs.relevance - rhs.relevance) < x) {
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus status = DocumentStatus::ACTUAL) const {
        ...
        //return FindTopDocuments(raw_query, [](int document_id, status, int rating) { return status == DocumentStatus::ACTUAL; });
    }
